      ...
      <div className="mt-7">
        <div className="px-4">
          ...
          <div className="flex items-center justify-between mb-3">
            <p className="font-manrope text-[11.5px] text-foreground/45">Blessed by the temple • Delivered to your door</p>
            <button onClick={() => navigate("/souvenir")} className="font-manrope text-[12px] font-semibold text-temple-gold shrink-0">View All →</button>
          </div>
        </div>
        ...
        <div className="overflow-x-auto pb-2" style={{ scrollSnapType: "x mandatory" }}>
          ...
          {BESTSELLERS.map((p, i) => (
            <div
              key={i}
              className="shrink-0 w-[148px] rounded-[18px] overflow-hidden flex flex-col"
              style={{ background: "#fff", border: "1px solid rgba(180,120,60,0.12)", boxShadow: "0 3px 16px rgba(100,50,15,0.08)", scrollSnapAlign: "start" }}
            >
              {/* Image */}
              <div className="relative h-[120px] overflow-hidden">
                ...
                {/* Wishlist */}
                <button
                  onClick={() => setWishlist((prev) => { const n = new Set(prev); n.has(i) ? n.delete(i) : n.add(i); return n; })}
                ...
                >
                  <svg viewBox="0 0 24 24" className="w-3.5 h-3.5" fill={wishlist.has(i) ? "#D97C2A" : "none"} stroke={wishlist.has(i) ? "#D97C2A" : "#888"} strokeWidth="2">
              ...
              <div className="p-3 flex flex-col gap-1.5 flex-1">
                <p className="font-manrope font-bold text-[12px] text-[#1C1C1C] leading-snug">{p.name}</p>
                <p className="font-manrope text-[10px] text-foreground/45 leading-none">{p.sub}</p>
                <div className="flex items-center justify-between mt-auto pt-1">
                  ...
                  <button
                    ...
                    style={{ background: addedToCart.has(i) ? saffron : "rgba(217,124,42,0.12)" }}
                  >
                    {addedToCart.has(i) ? (
                      ...
                      <svg viewBox="0 0 24 24" fill="none" stroke="#D97C2A" strokeWidth="2" className="w-3.5 h-3.5">
